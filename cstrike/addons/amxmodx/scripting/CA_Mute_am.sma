...
public plugin_natives() {
  register_native("ca_get_user_muted","_ca_get_user_muted");
  register_native("ca_set_user_muted","_ca_set_user_muted");
}

public _ca_get_user_muted(const iPlugin,const iParams) {
  enum {
    arg_id = 1,
    arg_player
  };
  ...
  return g_playersMute[id][pPlayer];
}

public _ca_set_user_muted(const iPlugin,const iParams) {
  enum {
    arg_id = 1,
    arg_player,
    arg_muted
  ...
  new id = get_param(arg_id);
  new pPlayer = get_param(arg_player);
  new bMuted = get_param(arg_muted);

  return g_playersMute[id][pPlayer] = bMuted ? true : false;
}
