...
Create_CVars() {
    bind_pcvar_string(create_cvar("pgb_storage_host", "127.0.0.1", FCVAR_PROTECTED,
        ...
        pgb_storage_host, charsmax(pgb_storage_host)
    )

    bind_pcvar_string(create_cvar("pgb_storage_user", "root", FCVAR_PROTECTED,
            .description = "PGBans MySQL database user"
        ),
        pgb_storage_user, charsmax(pgb_storage_user)
    )

    bind_pcvar_string(create_cvar("pgb_storage_pass", "pass", FCVAR_PROTECTED,
            .description = "PGBans MySQL database host password"
        ),
        pgb_storage_pass, charsmax(pgb_storage_pass)
    )

    bind_pcvar_string(create_cvar("pgb_storage_dbname", "db", FCVAR_PROTECTED,
            .description = "PGBans MySQL database name"
        ),
        pgb_storage_dbname, charsmax(pgb_storage_dbname)
    )
}
