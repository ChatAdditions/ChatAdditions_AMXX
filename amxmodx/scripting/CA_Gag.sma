...
new g_aGags_AdminEditor[MAX_PLAYERS + 1][gag_s];

new Array: g_aReasons, g_iArraySize_Reasons;
new Array: g_aGagTimes, g_iArraySize_GagTimes;

new bool: g_bStorageInitialized;
...
public Menu_Handler_SelectReason(id, menu, item) {
	...
	if(iReason == -1) {
		// не уверен насчёт разрушения меню при нажатии, но так как меню само по себе у клиента закроется, а ты его не отображаешь по новой -> меню нет
